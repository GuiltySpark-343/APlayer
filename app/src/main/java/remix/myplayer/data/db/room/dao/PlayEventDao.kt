...
interface PlayEventDao {
  ...
  @Query(
    ...
    WHERE eventType = 'playback' AND year = :year
      AND endReason IN ('SKIP_TO_NEXT', 'SKIP_TO_PREVIOUS')
    """
  )
  suspend fun countSkipped(year: Int): Int

  // 年度第一首 / 最后一首（按开始时间）
  @Query("SELECT * FROM play_events WHERE eventType = 'playback' AND year = :year ORDER BY startedAt ASC LIMIT 1")
  suspend fun firstEvent(year: Int): PlayEvent?

  @Query("SELECT * FROM play_events WHERE eventType = 'playback' AND year = :year ORDER BY startedAt DESC LIMIT 1")
  suspend fun lastEvent(year: Int): PlayEvent?
