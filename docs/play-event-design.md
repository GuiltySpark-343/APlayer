...
| **M2 报告层增强** | 时段分布 / 新增歌曲 / 循环王 / 连续听歌 / 深夜 / 跳过率 / 日历热力图 / 趋势曲线 / 分享卡片 / 年度歌单 / 多年对比 | ⬜ 未开始（见 12.2） |
| **M3 跨端互操作** | JSONL 导入（按 `eventId` 幂等）、多设备合并、PC/Postgres 落地 | ⬜ 未开始（见 12.1） |

### 0.3 当前实现到哪一层（一句话）

**实现到 M1（数据层）**：Android 端已能逐次采集播放事件、按 schema v2 存储（Room v9）、按年聚合、并导出 JSONL；报告层只提供了最小可用页面，其余呈现与跨端导入属于 M2/M3。
...
- **事件导入 / 跨设备同步**：当前仅支持导出 JSONL；按 `eventId` 幂等导入、多设备合并未实现（第 7、8 章标注为后续阶段）。
...
- **单曲循环王**：按 `loopCount > 1` 聚合。
- **连续听歌天数 / 最长连续**：按 `day` 与 `sessionId` 推导。
- **深夜听歌**：`hour \in [0,6)`。
- **跳过率 / 完整率**：按 `endReason` 与 `completed` 统计。
- **重复 vs 探索**：有效事件数 / 去重 `canonicalId`。
- **年度第一首 / 最后一首**：`MIN/MAX(startedAt)`。
- **其它呈现**：日历热力图、趋势曲线、分享卡片/长图、年度歌单一键生成、多年对比。
