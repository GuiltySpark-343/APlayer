...
**实现到 M1（数据层）+ M2 的 P0/P1（报告层）**：Android 端已能逐次采集播放事件、按 schema v2 存储（Room v9）、按年聚合、导出 JSONL；报告页已具备概览、TOP、月份趋势、时段/星期分布、听歌日历（含最长连续天数）、深夜、循环王、流派、首末曲、跳过/完整率、重复·探索、多年对比与一键生成年度歌单。**M2 剩余 P2（分享卡片/文案故事）与 M3（跨端导入/多设备合并）未开始。**
